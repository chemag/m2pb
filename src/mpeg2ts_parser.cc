...
Mpeg2TsParser::Mpeg2TsParser(bool return_raw_packets)
    : return_raw_packets_(return_raw_packets) {}

int Mpeg2TsParser::ParsePacket(int64_t pi, int64_t bi, const uint8_t *buf,
                               int len, Mpeg2Ts *mpeg2ts) {
  // clear input
  mpeg2ts->Clear();
...
int Mpeg2TsParser::DumpValidPacket(const Mpeg2TsPacket &mpeg2ts_packet,
  ...
  if (mpeg2ts_packet.header().adaptation_field_exists()) {
    res = DumpAdaptationField(mpeg2ts_packet.adaptation_field(), buf + bi,
                              len - bi);
    if (res < 0) {
      return -1;
    }
...
int Mpeg2TsParser::DumpHeader(const Mpeg2TsHeader &mpeg2ts_header, uint8_t *buf,
                              int len) {
  int bi = 0;
...
int Mpeg2TsParser::ParseAdaptationField(const uint8_t *buf, int len,
  ...
  if (adaptation_field_extension_flag) {
    res = ParseAdaptationFieldExtension(
        buf + bi, len - bi,
...
int Mpeg2TsParser::DumpAdaptationField(const AdaptationField &adaptation_field,
  ...
  if (adaptation_field.has_transport_private_data()) {
    ...
    bi += 1;
    res = adaptation_field.transport_private_data().copy((char *)(buf + bi),
                                                         len - bi);
    bi += res;
  }
  ...
  if (adaptation_field.has_adaptation_field_extension()) {
    res = DumpAdaptationFieldExtension(
        adaptation_field.adaptation_field_extension(), buf + bi, len - bi);
    if (res < 0) {
      return -1;
    }
    bi += res;
...
int Mpeg2TsParser::ParseAdaptationFieldExtension(
  ...
  if (piecewise_rate_flag) {
    int piecewise_rate =
        (buf[bi] & 0x3f) << 16 | (buf[bi + 1]) << 8 | (buf[bi + 2]);
  ...
  if (seamless_splice_flag) {
    ...
    int64_t dts_next_au =
        (((int64_t)(buf[bi + 0] & 0x0e) << 30) |
         ((int64_t)(buf[bi + 1]) << 22) |
         ((int64_t)(buf[bi + 2] & 0xfe) << 15) | ((int64_t)(buf[bi + 3]) << 7) |
...
int Mpeg2TsParser::DumpAdaptationFieldExtension(
  ...
  // add stuffing bytes
  while (bi <
         (1 + adaptation_field_extension.adaptation_field_extension_length())) {
    buf[bi++] = 0xff;
  }
  return bi;
}
...
int Mpeg2TsParser::ParsePCR(const uint8_t *buf, int len, PCR *pcr) {
  ...
  int64_t base = (((int64_t)(buf[0]) << 25) | ((int64_t)(buf[1]) << 17) |
                  ((int64_t)(buf[2]) << 9) | ((int64_t)(buf[3]) << 1) |
...
int Mpeg2TsParser::ParseESCR(const uint8_t *buf, int len, PCR *pcr) {
  ...
  // parse base/extension
  int64_t base =
      (((int64_t)(buf[0] & 0x38) << 27) |
       (((int64_t)(buf[0] & 0x03) << 28) | ((int64_t)(buf[1]) << 20) |
        ((int64_t)(buf[2] & 0xf8) << 12) | ((int64_t)(buf[2] & 0x03) << 13) |
        ((int64_t)(buf[3]) << 5) | (((int64_t)(buf[4]) & 0xf8) >> 3)));
  int extension = (((int)(buf[4] & 0x03) << 8) | (((int)(buf[5]) & 0xfe) >> 1));
  pcr->set_base(base);
  pcr->set_extension(extension);
  return 6;
}
...
int Mpeg2TsParser::ParseESRate(const uint8_t *buf, int len, int *es_rate) {
  ...
  *es_rate = (((int)(buf[0] & 0x7f) << 15) | (((int)(buf[1])) << 7) |
              (((int)(buf[2]) & 0xfe) >> 1));
  return 3;
}
...
int Mpeg2TsParser::ParsePTS(const uint8_t *buf, int len, int64_t *pts) {
  ...
  *pts =
      ((((int64_t)(buf[0]) & 0x0e) << 29) | (((int64_t)(buf[1]) & 0xff) << 22) |
       (((int64_t)(buf[2]) & 0xfe) << 14) | (((int64_t)(buf[3]) & 0xff) << 7) |
       (((int64_t)(buf[4]) & 0xfe) >> 1));
  // printf("found pts: %li\n", *pts);
  return 5;
}

int Mpeg2TsParser::DumpPTS(const int64_t pts, int start, uint8_t *buf,
                           int len) {
  int bi = 0;
...
int Mpeg2TsParser::ParsePesExtension(const uint8_t *buf, int len,
  ...
  if (p_std_buffer_flag) {
    ...
    int p_std_buffer_size = ((int)(buf[bi] & 0x1f) << 4) | ((int)(buf[bi + 1]));
  ...
  if (pes_extension_flag_2) {
    ...
    pes_extension->set_pes_extension_field(buf + bi,
...
int Mpeg2TsParser::DumpPesExtension(const PesExtension &pes_extension,
  ...
  if (pes_extension.has_pes_extension_field()) {
    ...
    bi += 1;
    res =
        pes_extension.pes_extension_field().copy((char *)(buf + bi), len - bi);
    bi += res;
  }
...
int Mpeg2TsParser::ParsePesPacket(const uint8_t *buf, int len,
  ...
  if (stream_id_type != PesPacket::STREAM_ID_PROGRAM_STREAM_MAP &&
    ...
    if (pes_crc_flag) {
      int previous_pes_packet_crc = (buf[bi] << 8) | buf[bi + 1];
...
int Mpeg2TsParser::DumpPesPacket(const PesPacket &pes_packet, uint8_t *buf,
                                 int len) {
  int bi = 0;
  int res;
...
int Mpeg2TsParser::ParsePsiPacket(const uint8_t *buf, int len,
  ...
  if (table_id == MPEG_TS_TABLE_ID_PROGRAM_ASSOCIATION_SECTION) {
    res = ParseProgramAssociationSection(
        buf + bi, len - bi, psi_packet->add_program_association_section());
    if (res < 0) {
    ...
    if (res < 0) {
      ...
      return -1;
             ...
             (table_id ==
              MPEG_TS_TABLE_ID_DVB_SERVICE_DESCRIPTION_TABLE_OTHER)) {
    res = ParseServiceDescriptionSection(
        buf + bi, len - bi, psi_packet->add_service_description_section());
    if (res < 0) {
...
int Mpeg2TsParser::DumpPsiPacket(const PsiPacket &psi_packet, uint8_t *buf,
                                 int len) {
  int bi = 0;
  int res;
  ...
  for (int i = 0; i < psi_packet.program_map_section_size(); ++i) {
    res = DumpProgramMapSection(psi_packet.program_map_section(i), buf + bi,
                                len - bi);
    if (res < 0) {
      return -1;
    }
  ...
  for (int i = 0; i < psi_packet.other_psi_section_size(); ++i) {
    res = DumpOtherPsiSection(psi_packet.other_psi_section(i), buf + bi,
                              len - bi);
    if (res < 0) {
      return -1;
    }
...
int Mpeg2TsParser::ParseProgramAssociationSection(
    const uint8_t *buf, int len,
  ...
  while (bi < (section_length - 4)) {
    int res = ParseProgramInformation(
        buf + bi, len - bi,
  ...
  int crc_32 =
      (((int32_t)(buf[bi + 0]) << 24) | ((int32_t)(buf[bi + 1]) << 16) |
       ((int32_t)(buf[bi + 2]) << 8) | ((int32_t)(buf[bi + 3])));
...
int Mpeg2TsParser::DumpProgramAssociationSection(
    const ProgramAssociationSection &program_association_section, uint8_t *buf,
    int len) {
  int bi = 0;
  int res;
...
int Mpeg2TsParser::ParseProgramInformation(
    const uint8_t *buf, int len, ProgramInformation *program_information) {
  int bi = 0;
  if ((len - bi) < 4) {
    return -1;
  }
...
int Mpeg2TsParser::DumpProgramInformation(
    const ProgramInformation &program_information, uint8_t *buf, int len) {
  int bi = 0;
...
int Mpeg2TsParser::ParseProgramMapSection(
    const uint8_t *buf, int len, ProgramMapSection *program_map_section) {
  int bi = 0;
  int res;
  if ((len - bi) < 8) {
    return -1;
  ...
  int current_next_indicator = (buf[bi] & 0x01);
  program_map_section->set_current_next_indicator(current_next_indicator);
  bi += 1;
  int section_number = buf[bi];
...
int Mpeg2TsParser::DumpProgramMapSection(
    const ProgramMapSection &program_map_section, uint8_t *buf, int len) {
  int bi = 0;
  int res;
  ...
  for (int i = 0; i < program_map_section.mpegts_descriptor_size(); ++i) {
    res = DumpDescriptor(program_map_section.mpegts_descriptor(i), buf + bi,
                         len - bi);
    if (res < 0) {
      return -1;
    }
...
int Mpeg2TsParser::ParseStreamDescription(
    const uint8_t *buf, int len, StreamDescription *stream_description) {
  int bi = 0;
  int res;
  if ((len - bi) < 5) {
    return -1;
...
int Mpeg2TsParser::DumpStreamDescription(
    const StreamDescription &stream_description, uint8_t *buf, int len) {
  int bi = 0;
  int res;
  ...
  for (int i = 0; i < stream_description.mpegts_descriptor_size(); ++i) {
    res = DumpDescriptor(stream_description.mpegts_descriptor(i), buf + bi,
                         len - bi);
    if (res < 0) {
      return -1;
    }
...
int Mpeg2TsParser::DumpOtherPsiSection(const OtherPsiSection &other_psi_section,
                                       uint8_t *buf, int len) {
  int bi = 0;
  int res;
...
int Mpeg2TsParser::ParseServiceDescriptionSection(
    const uint8_t *buf, int len,
  ...
  while (bi < (rem_length - 4)) {
    int res = ParseServiceDescription(
        buf + bi, len - bi,
...
int Mpeg2TsParser::DumpServiceDescriptionSection(
    const ServiceDescriptionSection &service_description_section, uint8_t *buf,
    int len) {
  int bi = 0;
  int res;
...
int Mpeg2TsParser::ParseServiceDescription(
    const uint8_t *buf, int len, ServiceDescription *service_description) {
  int bi = 0;
  if ((len - bi) < 4) {
    return -1;
  }
  ...
  service_description->set_descriptors_loop_length(descriptors_loop_length);
  bi += 2;
  // parse descriptors
  int fixed_bi = bi;
  while (bi < (fixed_bi + descriptors_loop_length)) {
    int res =
        ParseDescriptor(buf + bi, descriptors_loop_length - (bi - fixed_bi),
...
int Mpeg2TsParser::DumpServiceDescription(
    const ServiceDescription &service_description, uint8_t *buf, int len) {
  int bi = 0;
  int res;
  ...
  for (int i = 0; i < service_description.mpegts_descriptor_size(); ++i) {
    res = DumpDescriptor(service_description.mpegts_descriptor(i), buf + bi,
                         len - bi);
    if (res < 0) {
      return -1;
    }
